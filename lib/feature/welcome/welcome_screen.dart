...
class _WelcomeScreenState extends State<WelcomeScreen> {
  ...
  Future<void> _checkLoginStatus() async {
    ...
    if (isLoggedIn) {
      final email = await AuthSharedPrefs.getLoggedInUserEmail();
      if (email != null) {
        // Restorasi sesi login ke global Session
        final matchedUsers = userData.where((u) => u.email == email);
        if (matchedUsers.isNotEmpty) {
          Session.login(matchedUsers.first);
        }
      }
      if (!mounted) return;
      // Jika sudah login, arahkan ke Home Screen
                  ...
                  child: Column(
                    children: [
                      ...
                      SvgPicture.asset(
                        ...
                        placeholderBuilder: (context) => const Icon(Icons.movie_filter, color: _kOrange, size: 28),
                      ),
